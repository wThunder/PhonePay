<template>
	<view class="main">
		...
		<view class="tabs">
			<view class="tab-item" v-for="(A, B) in tabList" :key="B" :class="{ BB6: tabIndex == B }"
				@click="changeTabIndex(B)">{{ A }}</view>
		</view>
		...
		<view class="resList">
			<view class="item" v-for="(A, B) in resList" :key="B">
				<view class="top">
					...
					<view class="left" v-else-if="A.type == 2">提取</view>
		...
		<u-datetime-picker @cancel="clearPop" @confirm="ctimeOK" :min-date="etime" :max-date="Number(new Date())"
			:show="ctimeShow" v-model="ctime" mode="date"></u-datetime-picker>
		<u-datetime-picker @cancel="clearPop" @confirm="etimeOK" :min-date="Number(new Date()) - 1040688000000"
			:max-date="Number(new Date())" :show="etimeShow" v-model="etime" mode="date"></u-datetime-picker>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			getList(index) {
				let t = this;
				if (index == 0) {
					t.resList = t.resListOptions
					return
				}
				t.resList = t.resListOptions.filter(item => item.type == index)
			},
			getList2() {
